...
try {
    // 1. 이미 신청한 내역이 있는지 확인
    $check = $pdo->prepare("SELECT id FROM uedu_orders WHERE user_id = ? AND course_id = ?");
    ...
    $stmt = $pdo->prepare("INSERT INTO uedu_orders (user_id, course_id, amount, status, payment_key) VALUES (?, ?, ?, 'pending', ?)");
    $stmt->execute([$user_id, $data['course_id'], $data['amount'], 'BANK_' . time()]);
    
    // 3. 진도율 테이블 초기화 (미리 만들어두되 수강은 watch.php에서 막힘)
    $stmt = $pdo->prepare("INSERT INTO uedu_progress (user_id, course_id) VALUES (?, ?)");
